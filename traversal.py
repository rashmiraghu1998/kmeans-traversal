...
from pigg.pigg.core.evaluate import *
import time
...
async def lessThanGarbledCircuit(pid, p, kd_val_P1, kd_val_P2, point_1, point_2):
    circuit_name = "compare-lteqss-32-bit"
    # print("Time:"+str(time.ctime()) + "    * testing \"" + circuit_name + "\"...", end="")
    ...
    if pid == "P2":
        ...
        for i in range(len(str(kd_val_P2))):
            ...
            input_label_1 = await OT_input(pid, p, wire_to_labels_g[i][0], wire_to_labels_g[i][1], str(kd_val_P2)[i])
            input_labels.append(list(input_label))
            input_labels_points.append(list(input_label_1))
    ...
    if pid == "P0":
        ...
        gates_garbled = await p.get("gates_garbled_g")

        # All labels.
        wire_to_labels_P1.extend(wire_to_labels_P2)
        wire_to_labels_P1.extend(wire_to_labels_P1_2)
        wire_to_labels_P1.extend(wire_to_labels_P2_2)

        # Performed by P0.
        w_to_ls_e = evaluate.evaluate_gates(c, gates_garbled, wire_to_labels_P1)

        # Now, P1 and P0 need to evaluate the circuit to make P0 learn about the result.
        return garble.output_labels_to_bits(wire_to_labels_P1[:4], w_to_ls_e[:4])
...
async def sspir(pid, queue, P0_points, P1_points, P2_points, P1_kd_tree, P2_kd_tree, index):
    ...
    if pid == "P0":
        ...
        # print("Time:"+str(time.ctime()) + "  Secret: " + str(i_secret))  # Print the shares of 'i' for debugging
        ...
        # Q_xor_share = [Q, Q]
        # print("Time:"+str(time.ctime()) + "  XOR Q array: " + str(Q_xor_share))
    ...
    if pid == "P1":
        ...
        # print("Time:"+str(time.ctime()) + "  Received Q_P1: " + str(Q))
        # print("Time:"+str(time.ctime()) + "  Permuted W_P1: " + str(W))
        ...
        # print("Time:"+str(time.ctime()) + "  Computed V_P1: " + str(v_P1))
    ...
    if pid == "P2":
        ...
        # Permute the array Q using the value of i (secret share)
        W = [Q[(index + j) % len(Q)] for j in range(len(Q))]  # Permutation based on 'i'
        # print("Time:"+str(time.ctime()) + "  Received Q_P2: " + str(Q))
        # print("Time:"+str(time.ctime()) + "  Permuted W_P2: " + str(W))
        ...
        # print("Time:"+str(time.ctime()) + "  Computed V_P2: " + str(v_P2))

async def traversal(pid, queue, P0_points, P1_points, P2_points, P1_kd_tree, P2_kd_tree, index):

    print("Time:"+str(time.ctime()) + "  Started traversal....")
    starttime = time.time()
    p = utils.Message(queue)
    T1 = []
    T2 = []
    ...
    print("Time:"+str(time.ctime()) + "  P0: Shared the point")
    if pid == "P0":
        secret_datapoints = cryptographicProtocols.secret_share(P0_points, 2)
        # print("Time:"+str(time.ctime()) + "  Secret"+str(secret_datapoints))
        for i in range(len(P0_points)):
            bin_P1_points[i]= bin(int(bin_P1_points[i], 2) + int(secret_datapoints[0][i],2))[2:]
            bin_P2_points[i]= bin(int(bin_P2_points[i], 2) + int(secret_datapoints[0][i],2))[2:]
    ...
    print("Time:"+str(time.ctime()) + "  P0, P1 and P2 completed SSPIR")
    print("Time:"+str(time.ctime()) + "  Started less than garbled circuit")

    start_less = time.time()
    output = await lessThanGarbledCircuit(pid, p, kd_point_p1, kd_point_p2, bin_P1_points[0], bin_P2_points[0])
    end_less = time.time()
    print("Total time for less than GC: " +str(end_less - start_less))
    print("Time:"+str(time.ctime()) + "  Completed less than garbled circuit")
    print("Time:"+str(time.ctime()) + "  Start creating index vector")
    if pid == "P0":
    ...
    if pid == "P1":
        cp = await p.get("cp_1")
        T1.append(cp)
        print("Time:"+str(time.ctime()) + "  P1 stored the vector")
    if pid == "P2":
        cp = await p.get("cp_2")
        T2.append(cp)
        print("Time:"+str(time.ctime()) + "  P2 stored the vector")
